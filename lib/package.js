...
var debug = require('debug')('didi:install:package'),
...
var defulatPrefix = 'didi-component';

function Package(repo, ref, options) {
    debug('[initialize] %s@%s', repo, ref);
    options = options || {};
    options.prefix = options.prefix || defulatPrefix;
    options.root = options.root || path.join(util.getRoot()  || process.cwd(), 'component_modules');
    ...
    this.remotes = ['xiao' + 'ju','github'].map(function (r) {
...
proto.install = function (callback, dest) {
    var that = this;
    callback = callback || empty;
    ...
    this.resolve(function (err) {
        ...
        var ext = that.remote.archiveExt,
            ...
            dest = dest || path.join(that.root, that.name);
        ...
        that.download(that.archive, ball, function (err) {
            if (err) return callback.call(that, err);
            that.decompress(ball, archive, ext, function(err){
                if (err) return callback.call(that, err);
...
proto.decompress = function(ball, archive, ext, callback){
   var that = this; 
...
proto.download = function (url, target, callback) {
    callback = callback || empty;
    var that = this;
    var isOccurErr = false;
    // req.set('Accept-Encoding', 'gzip');
    debug('[download] from: %s', url);
    fis.util.write(target, '');
    ...
    function occurError (error){
        ...
        writer.destroy();
        debug('[download] failed: %s', target);

        return callback.call(that, error || 'occur 404 error');
    }
};
